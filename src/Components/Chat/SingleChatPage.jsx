...
const SingleChatPage = () => {
  ...
  const [contact, setContact] = useState({
    ...
    avatarUrl: "https://cdn.builder.io/api/v1/image/assets/TEMP/42b4dd3567b416fa17484e87d199c4c469070f73832ca08f0c0b5da31e673ac7?placeholderIfAbsent=true&apiKey=d8a8fe7915e44c6c92bb9b107a5f642c",
  });
  ...
  useEffect(() => {
    ...
    connection.on("ReceiveMessage", (senderId, content, imageUrl) => {
      ...
      const newMessage = {
        ...
        senderName: senderId === userData.id ? userData.firstName + " " + userData.lastName : contact.username,
        receiverName: senderId === userData.id ? contact.username : userData.firstName + " " + userData.lastName,
        img: imageUrl || null
      };
    ...
    const fetchMessages = async () => {
      try {
        ...
        if (response.data.status === "success") {
          ...
          const otherUser = messagesData[0]?.senderId === userData.id 
            ? { name: messagesData[0]?.receiverName }
            : { name: messagesData[0]?.senderName };
          const otherUserAvatar = messagesData[0]?.senderId === userData.id 
  ...
  return (
    <main className="flex flex-col items-center p-3 w-full bg-white dark:bg-gray-800 min-h-screen">
      <div className="w-full max-w-[1000px] mb-2 flex justify-between items-center">
        ...
        <h1 className="text-2xl font-bold text-sky-500 dark:text-sky-400 text-right">
          المحادثة مع {contact.username}
        </h1>
      </div>
      
      <article className="p-5 w-full bg-white dark:bg-gray-700 rounded-xl border border-black dark:border-gray-600 max-w-[1000px] shadow-md flex flex-col" style={{ height: "calc(100vh - 100px)" }}>
        <div className="flex gap-3 items-center justify-end border-b border-gray-200 dark:border-gray-500 pb-3 mb-3">
          <div className="flex gap-3 items-center">
            <h2 className="text-xl font-bold text-black dark:text-white text-right">{contact.username}</h2>
            <img
        ...
        <div className="flex-grow overflow-y-auto mb-4 p-2">
          {messages.length === 0 ? (
            ...
            messages.map((message) => (
              <div key={message.id} className={`mb-4 ${message.isCurrentUser ? 'flex justify-end' : 'flex justify-start'}`}>
                <div className={`flex max-w-[70%] ${message.isCurrentUser ? 'flex-row-reverse' : ''}`}>
                  <div className={`rounded-2xl p-3 ${message.isCurrentUser ? 'bg-sky-100 dark:bg-sky-800 dark:text-white' : 'bg-gray-100 dark:bg-gray-600 dark:text-white'}`}>
                    {message.content && (
                      <p className="text-sm text-right" dir="rtl">{message.content}</p>
                    )}
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-300 block mt-1 text-right">{message.timestamp}</span>
                  </div>
                </div>
              </div>
            ))
