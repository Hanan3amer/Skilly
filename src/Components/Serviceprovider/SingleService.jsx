...
function SingleService() {
  ...
  const handleCompleteService = async () => {
    try {
      ...
      if (!token) {
        toast.error("You must be logged in to complete this service");
        setCompletingService(false);
        return;
      }

      await axios.post(
        `https://skilly.runasp.net/api/Provider/ProviderServices/Complete-Service/${id}`,
        {},
        {
          headers: {
          ...
          {isCurrentWork ? (
            ...
            <>
              <button
                ...
                className="w-28 text-white bg-sky-500 hover:bg-sky-600 rounded-lg py-2 text-center"
              >
                تعديل
              </button>
              <button
                onClick={handleDeleteClick}
                className="w-28 text-white bg-red-500 hover:bg-red-600 rounded-lg py-2 text-center"
              >
                حذف
              </button>
            </>
