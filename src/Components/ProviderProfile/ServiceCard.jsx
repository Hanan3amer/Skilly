...
const ServiceCard = ({ 
  ...
  return (
    <Link to={`/${isGallery ? 'gallery' : 'service'}/${id}` + (isCurrentWork ? "?isCurrentWork=true" + (isRequest ? "&isRequest=true" : "") : "")} className="block w-full hover:shadow-lg transition-shadow rounded-xl overflow-hidden">
      <article className="flex flex-col h-full grow items-end py-10 px-9 w-full text-xs text-right rounded-xl bg-neutral-100 dark:bg-gray-700 transition-colors duration-300 max-md:px-5 max-md:py-6 max-md:mt-7 max-md:max-w-full">
            ...
            <div className="absolute bottom-4 right-4 z-10">
              <div className="flex items-center bg-[#24234C] rounded-xl px-3 py-1">
                ...
                <div className="bg-white rounded-full w-6 h-6 flex items-center justify-center">
                  <span className="text-[#24234C] text-sm font-bold">{offersCount}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="w-full flex justify-between items-start mt-3">
          ...
          <h3 className="text-sm md:text-base font-bold text-black dark:text-white">{title}</h3>
        </div>
