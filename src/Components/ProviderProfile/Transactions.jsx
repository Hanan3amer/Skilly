    ...
    <div className="w-full bg-[#F6F7F9] rounded-xl p-4 flex flex-col gap-1 shadow-sm">
      <div className="flex justify-end flex-row-reverse items-center mb-1">
        <div className="flex items-center gap-3">
          <span className="font-bold mr-2 text-gray-900 text-base">{userName}</span>
        </div>
        <span className="bg-sky-400 rounded-lg flex items-center justify-center w-10 h-10">
          <img src={transactionIcon} alt="transaction icon" className="w-6 h-6 bg-white rounded-full p-[3px]" />
        </span>
      </div>
      <div dir="rtl" className="text-gray-700 text-sm text-right leading-relaxed">
        {before}
        {amount && (
          <span className="text-sky-400 font-bold">{amount} {currency}</span>
        )}
        {after}
      </div>
      <div className="text-xs text-gray-300 text-left mt-1" dir="ltr">{formattedCreatedAt}</div>
    </div>
  );
};
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-[50%] max-w-[600px] min-w-[300px] p-0 relative">
        ...
        <div className="p-6 flex flex-col items-center">
          <div className="text-sky-400 font-bold text-lg mt-2 mb-1 text-center">رصيدك الحالي</div>
          <div className="text-black font-bold text-2xl mb-2 text-center">{balance !== null ? `${balance ?? 0} EGP` : <span className="text-gray-400 text-base">--</span>}</div>
          <hr className="w-full border-gray-200 my-2" />
          <div className="text-sky-400 font-bold text-lg mb-2 text-center">السحب</div>
          <form className="w-full flex flex-col items-center" onSubmit={handleSubmit}>
            <div className="flex w-full mb-4 gap-2 justify-center">
              <button type="button" onClick={() => setMethod("INSTAPAY")}
                className={`flex-1 py-2 rounded-lg font-bold transition-colors duration-150 ${method === "INSTAPAY" ? "bg-sky-400 text-white" : "bg-white text-sky-400   border border-sky-400"}`}>INSTAPAY</button>
              <button type="button" onClick={() => setMethod("محفظه")}
                className={`flex-1 py-2 rounded-lg font-bold transition-colors duration-150 ${method === "محفظه" ? "bg-sky-400 text-white" : "bg-white text-sky-400 border border-sky-400"}`}>محفظة</button>
            </div>
            {method === "INSTAPAY" && (
              <>
                <label className="block w-full text-right text-gray-700 text-sm mb-1 font-bold">عنوان الدفع</label>
                <div className="w-full mb-3 relative">
                  <input
                    type="text"
                    value={instapayEmail}
                    onChange={e => setInstapayEmail(e.target.value)}
                    ...
                    style={{ direction: 'rtl' }}
                  />
                  <span className="absolute right-4 top-1/2 border-l-2 border-gray-200 pl-1 -translate-y-1/2 text-gray-500 font-bold pointer-events-none select-none">@instapay</span>
                </div>
                <div className="text-center text-gray-400 my-2 font-bold">أو</div>
                <label className="block w-full text-right text-gray-700 text-sm mb-1 font-bold">رقم الهاتف</label>
                <input type="text" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)}
                  placeholder="رقم الهاتف" className="w-full mb-3 px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-sky-400 text-right bg-white placeholder-gray-400 font-bold" />
              </>
            )}
            {method === "محفظه" && (
              <>
...
export default function Transactions() {
  ...
  const displayedTransactions = expandedView ? allTransactions : allTransactions.slice(0, 6);
  ...
  const handleWithdraw = async ({ withdrawMethod, phoneNumber, instapayEmail }) => {
    setWithdrawLoading(true);
    try {
      const token = localStorage.getItem("userToken");
      await axios.post(
        ...
        {
          ...(withdrawMethod === "INSTAPAY" && { instapayEmail: instapayEmail ? `${instapayEmail}@instapay.com` :  phoneNumber ? phoneNumber :  undefined }),
          ...(withdrawMethod === "محفظه" && { phoneNumber: phoneNumber ? phoneNumber :  undefined }),
          withdrawMethod
        },
        {
          headers: {
  ...
  return (
    <section className="flex flex-col w-full text-gray-800 dark:text-white min-h-[300px]" id="transactions">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-xl w-[90%] max-w-[400px] p-0 relative">
            <div className="p-8 flex flex-col items-center text-center">
              ...
              <div className="text-gray-600 text-sm mb-6 leading-relaxed" dir="rtl">
      ...
      <div className="mt-2 w-full max-md:max-w-full">
        {loading ? (
          ...
          <div className="w-full text-center py-6">
            <p className="text-gray-500 dark:text-gray-400">لا توجد معاملات حالياً</p>
          </div>
        ) : (
          <div className="flex flex-col gap-8 w-full">
            {Object.entries(groupedDisplayed).map(([groupTitle, groupTransactions]) => (
              <div key={groupTitle} className="flex flex-col gap-4">
                <div className="text-gray-400 font-bold text-base mb-1 text-right pr-2">{groupTitle}</div>
                <div className="grid grid-cols-2 gap-5 max-md:grid-cols-1 w-full justify-items-end" dir="rtl">
