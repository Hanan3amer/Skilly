...
export default function User() {
  ...
  useEffect(() => {
    if (isEditMode) {
      formik.setValues({
        Governorate: editUser.governorate || "",
        City: editUser.city || "",
        StreetName: editUser.streetName || "",
        Gender: editUser.gender?.toString() || "1",
      });
      setUserinfo({
      ...
      setPreviewImage(editUser.img || null);
      setActiveOption(editUser.gender === 0 ? "female" : "male");
      setCities(governoratesWithCities[editUser.governorate] || []);
    }
  }, [editUser]);
  ...
  return (
    <div className="container my-10">
      ...
      >
        <h1 className="text-center text-[#3B9DD2] font-extrabold text-2xl my-2">
          {isEditMode ? "تعديل البيانات الشخصية" : "بيانات المستخدم"}
        </h1>
        <h5 className="text-gray-900 font-bold text-sm text-center">
          برجاء {isEditMode ? "تحديث" : "إكمال"} بياناتك بعناية
        </h5>

        <div className="border my-5 p-5 rounded-lg">
          <label
            htmlFor="Img"
            className="flex text-[#23255B] border border-[#23255B] rounded-full text-base px-5 py-5 outline-none w-max cursor-pointer mx-auto"
          >
            {previewImage ? (
              <img
                src={previewImage}
